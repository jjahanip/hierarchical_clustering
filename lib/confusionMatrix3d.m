function [confusion_matrix overall_pcc group_stats groups_list] = confusionMatrix3d(predicted_groups, actual_groups, biomarker)
% confusionMatrix3d
%
...
groups_list = {strcat(biomarker,'-'), strcat(biomarker,'+')};
set(gca,'YTickLabel',groups_list);
set(gca,'XTickLabel',groups_list);


x_h = ylabel('Actual Group', 'FontSize', 12, 'FontWeight', 'bold');
y_h = xlabel('Predicted Group', 'FontSize', 12, 'FontWeight', 'bold');
zlabel('Number of Observations', 'FontSize', 12, 'FontWeight', 'bold');
acc_percent = round(overall_pcc*100,2);
text(0.73, 0.88, ['Overall Acuracy: ' num2str(acc_percent) '%'],...
     'units', 'normalized',  'FontSize', 12, 'FontWeight', 'bold');
title({strcat(biomarker, ' classification')},'fontsize',14);
